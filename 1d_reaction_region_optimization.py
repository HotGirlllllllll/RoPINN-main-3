...
if not os.path.exists('./results/'):
    os.makedirs('./results/')
torch.save(model.state_dict(), tagged_path(f'./results/1dreaction_{args.model}_region.pt'))
...
if args.paper_outputs:
    ...
    np.savetxt(
        tagged_path(f'./results/1dreaction_{args.model}_region_loss.csv'),
        loss_dump,
        delimiter=',',
    ...
    plt.tight_layout()
    plt.savefig(tagged_path(f'./results/1dreaction_{args.model}_region_optimization_loss.pdf'), bbox_inches='tight')
...
if args.paper_outputs:
    with open(tagged_path(f'./results/1dreaction_{args.model}_region_metrics.csv'), 'w', encoding='utf-8') as fp:
...
plt.tight_layout()
plt.axis('off')
plt.savefig(tagged_path(f'./results/1dreaction_{args.model}_region_optimization_pred.pdf'), bbox_inches='tight')
...
plt.tight_layout()
plt.axis('off')
plt.savefig(tagged_path('./results/1dreaction_exact.pdf'), bbox_inches='tight')
...
plt.tight_layout()
plt.axis('off')
plt.savefig(tagged_path(f'./results/1dreaction_{args.model}_region_optimization_error.pdf'), bbox_inches='tight')
